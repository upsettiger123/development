...
The HikeTile component is passed the individual hike item data, updateHike, and whether the tile has already been added to the aggregator from FilteredList. The HikeTile component includes a button that calls the updateHikes method to modify that specific hike's data so that the aggregator can use the updated information. The individual hike item data is displayed as a card and the type of button (outlined or colored) is determined by the added prop (which is true if added to the aggregator and false otherwise).
